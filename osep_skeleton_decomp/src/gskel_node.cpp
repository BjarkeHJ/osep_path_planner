...
GSkelNode::GSkelNode() : Node("GSkelNode") {
    ...
    gskel_cfg.fuse_dist_th = declare_parameter<float>("gskel_fuse_dist_th", 2.0f);
    ...
    gskel_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("/global_skeleton", pub_qos);
...
void GSkelNode::publish_gskel(pcl::PointCloud<pcl::PointXYZ>::Ptr& gskel_out, const std_msgs::msg::Header& src_header) {
    if (!gskel_out || gskel_out->points.empty()) return;
    sensor_msgs::msg::PointCloud2 gskel_msg;
    pcl::toROSMsg(*gskel_out, gskel_msg);
    gskel_msg.header = src_header;
    if (gskel_msg.header.frame_id.empty()) gskel_msg.header.frame_id = "odom";
...
void GSkelNode::process_tick() {
    ...
    // Run GSkel
    if (gskel_->gskel_run()) {
        std::cout << "[SUCCESS] Ran GSkel Algorithm..." << std::endl;
        global_skeleton = gskel_->output_gskel();
        publish_gskel(global_skeleton, gmsg.header);
    }
}
